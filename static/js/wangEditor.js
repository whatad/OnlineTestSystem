...
_e(function (E, $) {
    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        $txt.on('paste', function (e) {
            ...
            if (editor.config.noPaste) {
                alert ('禁止粘贴操作！');
                return false;
            } else if (editor.config.pasteText) {
                // 只粘贴纯文本
    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (!resultText.success) {
                // 提示错误
                E.warn('上传失败：' + resultText.desc);
                alert(resultText.desc);
            } else {
                E.log('上传成功，即将插入编辑区域，结果为：' + resultText.bizContent);
                img = document.createElement('img');
                img.onload = function () {
                    // 将结果插入编辑器
                    var html = '<img src="' + resultText.bizContent.url + '" alt="' + originalName + '" style="max-width:100%;"/>';
                    editor.command(null, 'insertHtml', html);

                    E.log('已插入图片，地址 ' + resultText.bizContent.url);
                    img = null;
                };
                img.onerror = function () {
                    E.error('使用返回的结果获取图片，发生错误。请确认以下结果是否正确：' + resultText.bizContent);
                    img = null;
                };
                img.src = resultText.bizContent.url;
            }
